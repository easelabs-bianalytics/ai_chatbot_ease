...
- [ ] Usuário somente leitura criado no RDS.
- [ ] `GRANT SELECT` limitado aos objetos usados pelo catálogo, incluindo a
  função `ruptura_extrato.f_haversine_km` (Q22).
...
- [ ] Tabelas que existem mas **não** devem ser usadas, e qual usar no lugar
  (como já feito com `cddd.fato_cdd` → `vendas_consolidado`).
...
- [ ] Período padrão ou esclarecimento obrigatório (O-04).
